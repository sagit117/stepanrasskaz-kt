...
export default class Email {
    ...
    #confirmationEmail = () => {
        ...
        Api.confirmationEmailGetCode(this.#emailData.id)
            .then((res) => {
            if (res.ok) {
                return new Toast("Код подтверждения отправлен", "На указанный email отправлен код подтверждения", "SUCCESS", 3)
                    .render("toasts");
            }
            ...
            switch (error) {
                default:
                    new Toast("Ошибка", "Произошла внутренняя ошибка сервера", "ERROR", 3)
                        .render("toasts");
            }
        });
    };
